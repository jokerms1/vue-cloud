...
<!--      <input type="file" id="downloadfile" directory style="display: none" @change="getdownloadFile">-->
      ...
      <Button @click="newFolderPushToFileList"><Icon type="ios-folder-outline"></Icon>新建文件夹</Button>
      <Button @click="deleteContainer">删除云盘</Button>
      ...
      <div  class="body">
        <div v-for="(item,index) in uploadlist" class="fileitem">
          <FileItem
            ...
            :index="index"
            :method="item.method" @loadFileLists="loadFileList">
          </FileItem>
        </div>
      </div>
    </el-card>
...
<script>
  ...
  import SparkMD5 from 'spark-md5'
    export default {
      ...
      data(){
        return{
            ...
            chunk_size:4096,
            file:null,
            downloadfile:null,      //下载的文件
            downloadpath_des:null,  //下载文件的目标路径
            downloadpath_src:null,  //下载文件的源路径
            downloadtype:null,      //下载文件的类型
            cur_count:-1,            //当前文件的切片数
            container:null,
            fileList:[],
            max_count:null,
            lock:true,
            ...
            breadList:[{absolutepath:'',path:'', name:'全部文件'}],
            showBackToPre:false,
      ...
      methods:{
          ...
          //加载文件列表
          loadFileList(){
              if(this.$cookies.get('container') == null){
                  console.log("container is null");
                  return;
              }
              //获取到当前路径下的文件列表
              this.axios.get('listfiles',{
                  params:{
                      container:this.$cookies.get('container'),
                      path:this.breadList[this.breadList.length-1].absolutepath,
                  }
              }).then((res)=>{
                  ...
                  for(let i = 0 ; i <= res.data.length - 1; i++){
                      ...
                      if(res.data[i].isdir == true){
                          let type = 'folder'
                          filelist.push({fileName:filename,size:res.data[i].size,path:res.data[i].path,time:res.data[i].lastModified,isDir:'文件',type:type});
                      }else{
          ...
          //加载面包屑
          loadBreadList(){
              console.log(this.$route.query.container.container);
              //判断route里面的container不能为空、不能为undefined、不能为null
            if(this.$route.query.container.container !== null  && this.$route.query.container.container !== "" && this.$route.query.container.container !== undefined){
                this.$cookies.set('container',this.$route.query.container.container);
            }
            this.loadFileList();
          },
          //初始化容器
          initContainerPage(){
              this.container = this.$route.query.container.container;
              console.log("container:"+this.container);
          },
          //删除容器
          deleteContainer(){
              this.axios.get('deletecontainer',{
                  params:{
          ...
          uploadFile(file){
              ...
              let des;
              if(this.breadList.length == 1){
                  des = path2[path2.length - 1];
              }else{
                  des = this.breadList[this.breadList.length-1].absolutepath + '/' + path2[path2.length - 1];
              }
              console.log("des:"+des);
              var url = this.URL  + "upload?" + "path_src="+ _path + "&path_des="  + des + "&container=" + this.$cookies.get('container')
                  +"&md5=" + this.tmp_md5 + "&size=" + this.size + "&count="+ this.cur_count
                  +"&maxcount=" + this.max_count +"&token="+_token;
              console.log("url:"+url);
              var list = {path_src:_path,file:this.file,url:url,count:0,maxcount:this.max_count,method:"upload"}
          ...
          downloadFile(path,type){
              let container = this.$cookies.get('container');
              let _token =  this.$cookies.get('token');
              //console.log(path);
              window.location.href ="http://192.168.0.48:8080/download?" + "path=" + path + "&container=" + container + "&token=" + _token;
          },
          //初始化socket
          initSocket(){
    ...
    mounted(){
          $(".box-card").hide();
          this.initContainerPage();
          // console.log($route.query.container.container);
          // this.container = $route.query.container.container;
          // console.log("container:" + this.$route.query.container.container);
          this.loadBreadList();
          ///this.delete();
    },
    //监听数据的变化
    watch:{
        ...
        'downloadlist':function (newVal,oldVal) {
            //监听downloadlist的数据的变化，弹出传输列表框
            $(".box-card").show();
        },
        '$route.query.container.container':function(){
            this.initContainerPage();
        }
    },
    distroyed(){
